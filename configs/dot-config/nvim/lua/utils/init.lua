...
function M.empty_map_table()
  local maps = {}
  for human_mode, _ in pairs(mode_adapters) do
    maps[human_mode] = {}
  end
...
function M.set_keymaps(map_table, base)
  -- iterate over the first keys for each mode
  base = base or {}
  for mode, maps in pairs(map_table) do
    -- iterate over each keybinding set in the current mode
    local short_mode = mode_adapters[mode]
    for keymap, options in pairs(maps) do
      ...
      if options then
        ...
        if not cmd or keymap_opts.name then -- if which-key mapping, queue it
          ...
          if not M.which_key_queue[short_mode] then M.which_key_queue[short_mode] = {} end
          M.which_key_queue[short_mode][keymap] = keymap_opts
        else -- if not which-key mapping, set it
          vim.keymap.set(short_mode, keymap, cmd, keymap_opts)
        end
      end
    end
  end
